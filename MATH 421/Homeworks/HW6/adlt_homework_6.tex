...
I attended collaborative office hours and discussed Problem 6 with Professor Grace Work. Her guidance helped me better understand the structure of the proof and how to apply the Squeeze Lemma. All written work and final write-up are my own.
...
\textbf{Proof.} \\ 
Let $S \subseteq \mathbb{R}$ be a bounded nonempty set such that $\sup S \notin S$. Let $t = \sup S$. Since $t$ is the least upper bound of $S$, for every $\varepsilon > 0$, then there exists an element $s \in S$ such that $t - \varepsilon < s \leq t$. \\
